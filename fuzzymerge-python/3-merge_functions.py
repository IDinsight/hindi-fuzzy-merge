...
def merge_data(left_data_df,
    ...
    if (age_limits != "None"):
        ...
        results_df['right_age_high'] = results_df['right_age'] + \
            int(age_limits)
        results_df = results_df[results_df.left_age.between(
            results_df.right_age_low, results_df.right_age_high)]

        # Keep rows for each unique id for which the age is closest in the 2 datasets
        results_df['right_age_diff'] = abs(
            results_df['right_age'] - results_df['left_age'])
        results_df = results_df[results_df['right_age_diff'] == results_df.groupby(
            'left_dataset_unique_id')['right_age_diff'].transform('min')]
