...
def fix_transliterations_level_1(p_word):
    ...
    v_word_upd = v_word_upd.replace("SH", "S")
...
def soundex(p_word):
    ...
    if len(v_letters) > 0:
        ...
        v_letters = [
            v_char for v_char in v_letters if v_char not in v_to_remove]
    ...
    if len(v_letters) > 0:
        ...
        v_letters = [
            v_char for v_char in v_letters if isinstance(v_char, int)][0:3]
...
def get_first_letter_key(p_soundex_key):
    ...
    v_first_letter_key = [v_value for v_group, v_value in v_groups.items(
    ) if v_soundex_key_letter in v_group]
