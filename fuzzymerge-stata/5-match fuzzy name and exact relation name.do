...
// Using stricter thresholds than (3) since more important that name matches

	* Specifying last non-merge datasets to start loop
	local census_nmerge "$clean_dta/census_nmerge4"
	local voter_nmerge "$clean_dta/voter_nmerge4"
	local z 1 // Version 
			...
			foreach relation in father spouse{
				...
				* Temporary save
					...
					* Keep merges
					...
					gen merge_type = "5-`z'(fuzzy name,exact `relation' name),v`i' names,matchscore > 0.`m'"
					local ++z
	...
	* Append merge files and save
	...
	save "$clean_dta/merge5", replace

	* Remainder Census
	use `census_nmerge', clear
	save "$clean_dta/census_nmerge5", replace

	* Remainder Voter
	use `voter_nmerge', clear
	save "$clean_dta/voter_nmerge5", replace
