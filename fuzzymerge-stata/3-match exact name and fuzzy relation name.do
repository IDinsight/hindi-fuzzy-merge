...
// Run twice, first with stricter threshold on each of 6 names (matchscore > 0.9), then on looser threshold (matchscore > 0.8) 
	...
	* Specifying last non-merge datasets to start loop
	local census_nmerge "$clean_dta/census_nmerge2"
	local voter_nmerge "$clean_dta/voter_nmerge2"
	local z 1 // Version 
	...
	* Loop over two matchscore thresholds (stricter first, then looser)
		...
		forval i = 1/6{ 
			...
			* Loop over 2 relations
			foreach relation in father spouse {
				...
				* Merge
				reclink psid name`i' `relation'_name`i' using `census_working', idmaster(voter_uniqid) idusing(census_uniqid) gen(matchscore) required(psid name`i') wmatch(1 1 10)		
				...
				* Temporary save
					...
					* Keep merges
					...
					gen merge_type = "3-`z'(exact name,fuzzy `relation' name),v`i' names,matchscore > 0.`m'"
					local ++z
	...
	//PENDING Following shows error invalid file specification
	...
	* Append merge files and save
	...
	save "$clean_dta/merge3", replace

	* Remainder Census
	use `census_nmerge', clear
	save "$clean_dta/census_nmerge3", replace

	* Remainder Voter
	use `voter_nmerge', clear
	save "$clean_dta/voter_nmerge3", replace
