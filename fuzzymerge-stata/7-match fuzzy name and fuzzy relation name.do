...
// Run twice, first with stricter threshold on each of 6 names (matchscore > 0.9), then on looser threshold (matchscore > 0.8) 
	...
	* Specifying last non-merge datasets to start loop
	local census_nmerge "$clean_dta/census_nmerge6"
	local voter_nmerge "$clean_dta/voter_nmerge6"
	local z 1 // Version 
		...
		forval i = 1/6{
			...
			* Loop over 2 relations
			foreach relation in father spouse{
				...
				* Merge
				reclink psid `relation'_name`i' name`i' using `census_working', idmaster(voter_uniqid) idusing(census_uniqid) gen(matchscore) required(psid) wmatch(1 10 20)
				...
				* Keep good merges 
					...
					* Drop matches with big differences in lengths - reclink doesn't weight length similarity enough
					gen Lname`i' = strlen(subinstr(name`i'," ","",.))
					gen LUname`i' = strlen(subinstr(Uname`i'," ","",.))
					gen length_diff = abs(Lname`i'-LUname`i')
					drop if length_diff > 3
					drop length_diff L*

					* Drop matches with big differences in lengths - reclink doesn't weight length similarity enough
					gen L`relation'_name`i' = strlen(subinstr(`relation'_name`i'," ","",.))
					gen LU`relation'_name`i' = strlen(subinstr(U`relation'_name`i'," ","",.))
				...
				* Temporary save
					...
					* Keep merges
					...
					gen merge_type = "7-`z'(fuzzy name,fuzzy `relation' name),v`i' names,matchscore > 0.`m'"
					local ++z
	...
	* Append merge files and save
	...
	save "$clean_dta/merge7", replace

	* Remainder Census
	use `census_nmerge', clear
	save "$clean_dta/census_nmerge7", replace

	* Remainder Voter
	use `voter_nmerge', clear
	save "$clean_dta/voter_nmerge7", replace
